...
const STORAGE_KEY = 'projects_last_sync_time';
...
export const useSyncProjects = () => {
  ...
  const syncNow = useCallback(async (force: boolean = false): Promise<SyncResult> => {
    if (!force && !canSync()) {
      ...
      return {
        success: false,
        message: `Синхронизация проектов возможна через ${minutesRemaining} мин.`
      };
    }
    ...
    if (isSyncing) {
      return {
        success: false,
        message: 'Синхронизация проектов уже выполняется'
      };
    }
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('sync-projects-sheets');
      ...
      if (result.success) {
        ...
        console.log('Projects sync completed:', result);
      }
  ...
  const syncOnAppLoad = useCallback(async () => {
    if (canSync()) {
      console.log('Projects auto-sync on app load...');
