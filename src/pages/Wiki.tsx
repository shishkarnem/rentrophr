...
const renderTextWithLinks = (text: string) => {
  ...
  return parts.map((part, index) => {
    if (urlRegex.test(part)) {
      ...
      return <a key={index} href={part} target="_blank" rel="noopener noreferrer" className="text-accent hover:text-accent/80 underline break-all transition-colors">
          {part}
        </a>;
    }
    return part;
    ...
    const fetchFaq = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('faq_knowledge').select('id, question, answer, question_en, question_kz, answer_en, answer_kz, category, search_keywords').order('created_at', {
          ascending: true
        });
  ...
  const performAiSearch = async (query: string) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('wiki-ai-search', {
        body: {
          query,
          language,
          faqItems: faqItems.map(f => ({
            question: getQuestion(f),
            answer: getAnswer(f),
            category: f.category
          }))
        }
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          duration: 0.5
            ...
            <p className="text-white/60 max-w-2xl mx-auto">
              {language === 'ru' ? 'База знаний с ответами на часто задаваемые вопросы' : language === 'en' ? 'Knowledge base with answers to frequently asked questions' : 'Жиі қойылатын сұрақтарға жауаптары бар білім базасы'}
            </p>
          </motion.div>
          ...
          delay: 0.1
            ...
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={language === 'ru' ? 'Поиск по вопросам...' : language === 'en' ? 'Search questions...' : 'Сұрақтарды іздеу...'} className="w-full pl-12 pr-12 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:border-accent/50 transition-colors" />
              {isAiSearching && <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-accent animate-spin" />}
            </div>
          </motion.div>

          {/* AI Search Result */}
          <AnimatePresence>
            {aiSearchResult && <motion.div initial={{
            opacity: 0,
            y: -10,
            height: 0
          }} animate={{
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            {isLoading ? <div className="text-center py-12">
              ...
              </div> : Object.keys(groupedItems).length === 0 ? <motion.div initial={{
            opacity: 0
          }} animate={{
            opacity: 1
                ...
                <p className="text-white/60">
                  {searchQuery ? language === 'ru' ? 'Ничего не найдено' : language === 'en' ? 'Nothing found' : 'Ештеңе табылмады' : language === 'ru' ? 'Пока нет вопросов в базе знаний' : language === 'en' ? 'No questions in knowledge base yet' : 'Білім базасында әлі сұрақтар жоқ'}
                </p>
              </motion.div> : Object.entries(groupedItems).map(([category, items], categoryIndex) => <motion.div key={category} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: categoryIndex * 0.1
                ...
                delay: index * 0.05
                        ...
                        <div className="glass-dark rounded-xl overflow-hidden border border-white/10 hover:border-accent/30 transition-colors">
                          <button onClick={() => toggleExpand(item.id)} className="w-full flex items-center justify-between p-4 md:p-5 text-left">
                            ...
                            <span className="flex-shrink-0 w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center">
                              {expandedId === item.id ? <ChevronUp className="w-5 h-5 text-accent" /> : <ChevronDown className="w-5 h-5 text-white/60" />}
                            </span>
                          </button>
                          
                          <AnimatePresence>
                            {expandedId === item.id && <motion.div initial={{
                      height: 0,
                      opacity: 0
                    }} animate={{
                      height: 'auto',
